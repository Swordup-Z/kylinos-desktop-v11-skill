...
- Local source customizations may be committed in the local worktree to preserve a clear change boundary, but they should not be pushed by default. Export the commit as a patch under `/data/usershare/kylinos-local-sources/<component-or-fix>/patches/<source-package>/`, which belongs to the local customization project root and sits alongside `<source-tree>/`, `build/`, and `rollback/`; group patches by source-package name, and maintain the project-level mapping in `/data/usershare/kylinos-local-sources/README.md` plus the source-package, patch, installed-file, and rollback mapping in `CUSTOMIZATION.md` so the customization can be found, reapplied, or migrated after system package upgrades.
- When a local customization source repository already exists and needs to be updated later, update it in place with `fetch`, branch switches, or new branches plus patch migration. Do not delete the directory or reclone the whole repository by default; create a replacement clone only when the repository is damaged, the remote is wrong, required history cannot be fetched, or the user explicitly asks.
- Source-rebuild repairs must distinguish stages: a successful build plus ABI/RPATH/symbol checks only means `build-verified` or `staged-for-install`, not fixed. Mark a change as `runtime-verified` only after it has been installed into the real system and verified in the actual desktop, service, and reboot/relogin scenarios.
...
- UKUI global-search custom command providers, such as adding system actions like “Empty Trash” through a user-level JSON configuration or a graphical settings entry: [`references/ukui-search.md`](references/ukui-search.md), [`knowledge/source-rebuild/ukui-search-command-provider.md`](knowledge/source-rebuild/ukui-search-command-provider.md)
