...
- 需求类型：系统问题修复。
- 场景：UKUI 打开方式文件选择器。
- 功能目标：修复“打开方式 -> 选择其他应用”进入 `/usr/share/applications` 后应用 `.desktop` 条目被错误过滤的问题。
...
- 本地验证提交：`6b964c54212b6ee4a714d06d5401ac4feef7965a`
- 运行时验证状态：`runtime-verified`
- 验证结果：在保持系统原始 `libpeony` 的前提下，修复 `/usr/share/applications` 应用条目过滤后，“打开方式 -> 更多应用 -> 选择其他应用”中的真实应用和图标均可见。
...
该 patch 只改变 UKUI 原生文件选择器在 `/usr/share/applications` 下处理 `*.desktop` 过滤的行为：
...
这样可以避免 Peony 使用桌面文件本地化显示名去匹配 `*.desktop`，导致真实应用条目被隐藏。
...
1. 先判断新版本是否已经修复了 `/usr/share/applications` 下 `*.desktop` 名称过滤。
2. 如果函数名或文件位置变化，迁移两段语义：目录切换后重放过滤；只在应用 desktop 目录清空实际名称过滤。
...
运行时打开“打开方式 -> 选择其他应用”，确认 `/usr/share/applications` 下真实应用和图标可见。
