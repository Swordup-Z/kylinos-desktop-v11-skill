...
这是 KylinOS Desktop V11 桌面系统问题修复经验库。只要用户问题属于“系统已有行为异常、失效、报错、不能持久化、安装失败或系统服务损坏”，就先读取本入口；再按实际场景读取 `references/system-repair/<scenario>.md`。如果没有命中专门 reference，至少读取 `references/system-repair/system.md`。
...
- 如果问题未命中某个专门 reference，读取 [references/system-repair/system.md](references/system-repair/system.md)，再继续诊断。
...
- 遵循渐进式披露：先读本入口判断修复场景；再只读取命中的 `references/system-repair/<scenario>.md`；随后读取该 reference 指向的 `knowledge/system-repair/<scenario>/README.md`；最后只读取与用户问题匹配的具体知识文件。不要把未命中的 reference、场景 README 或 knowledge 预加载进上下文。
- `references/` 定位为系统修复场景分类的路由入口和快速诊断索引。一级目录只保留 `system-repair/`；二级文件按场景命名，例如 `ukui.md`、`network.md`、`source-rebuild.md`。命名和路由规则见 [references/README.md](references/README.md)。
- `knowledge/` 定位为可复用的具体修复章节。一级目录只保留 `system-repair/`；二级目录与 references 使用相同场景分类。结构说明见 [knowledge/README.md](knowledge/README.md)。
- 新增经验时落到最小匹配场景的 `knowledge/system-repair/<scenario>/` 章节；若还没有对应章节，新增章节后从同场景 README 链接过去。维护模式、安全边界、通用诊断闭环属于 `system-repair/system/`。
- 如果修复问题必须重新编译系统源码包或替换系统共享库，先读取 [references/system-repair/source-rebuild.md](references/system-repair/source-rebuild.md)；如果目标是源码级功能增强或本地客制化，切换到 `$HOME/.os-enhance-skill/SKILL.md`。
...
- 如果实际解决的是当前 skill 尚未覆盖的 KylinOS Desktop V11 系统修复问题，应新增合适的 `references/system-repair/<scenario>.md` 作为分类入口，并把详细处理经验沉淀到同场景的 `knowledge/system-repair/` 章节；再在 `SKILL.md` 的“参考文档路由”中补充入口。
...
按实际修复场景读取最小 reference；只有未命中时才读取 `references/system-repair/system.md`。
...
具体应用安装、AppImage、KARE 环境误装和第三方 apt 源处理步骤不要堆在本入口中；按需读取 [references/system-repair/applications.md](references/system-repair/applications.md)。
